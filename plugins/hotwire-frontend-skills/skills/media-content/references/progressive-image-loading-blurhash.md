...
1. Canvas dimensions must match the image dimensions. Use `this.imageTarget.width` and `this.imageTarget.height` to set the canvas size. **Important:** The `<img>` tag must have explicit `width` and `height` HTML attributes, otherwise these properties return 0 before the image loads and the blurhash canvas will be empty.
