...
To also update the browser URL during Turbo Frame navigation, use `data-turbo-action="advance"` or `data-turbo-action="replace"` on the frame element. This promotes the frame navigation to a page-level history entry.
...
**BAD - No scroll preservation, causing scroll-to-top on back navigation**:

```javascript
// No turbo:before-cache listener — scroll position is lost when
// Turbo caches the page snapshot, and the user is scrolled to the
// top on every back/forward navigation.
```
